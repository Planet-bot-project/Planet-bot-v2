...
async function generatePomodoroPicture(mode, pomodoroState) {
	// 全引数が必須
	if (!mode || !pomodoroState) {
		console.log("Error: All arguments (mode, pomodoroState) are required.");
		return;
	}
	...
	try {
		...
		// タイトル
		ctx.fillStyle = "#fff";
		registerFont("./assets/fonts/NotoSansJP-Bold.ttf", {  // registerFontの仕様を考慮してパス指定
			family: "NotoSansJP",
		});
