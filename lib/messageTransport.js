...
//メッセージ展開
// 引数：　client:discordBOTのクライアント,message:転送対象のメッセージ,sendChannelId:転送先チャンネルID
async function messageTransport(client, message, sendChannelId) {
  const {
    ActionRowBuilder,
    ButtonBuilder,
    ButtonStyle,
  ...
  try {
    ...
    const sendChannel = await client.channels.fetch(sendChannelId);
    sendChannel.send({
