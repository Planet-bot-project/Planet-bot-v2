...
async function create(audioType, speakerId) {
  ...
  if (audioType == "startWorking") {
    filePath = `./Audio/${speakerId}/startWorking.wav`;
    text = "作業時間が始まります。集中して取り組んでください。";
  } else if (audioType == "startBreaking") {
    filePath = `./Audio/${speakerId}/startBreaking.wav`;
    text = "休憩時間が始まります。リラックスして休んでください。";
  } else if (audioType == "startLongBreaking") {
    filePath = `./Audio/${speakerId}/startLongBreaking.wav`;
  ...
  // ディレクトリの確認。なければ作成
  if (!fs.existsSync(`./Audio/${speakerId}`))
    fs.mkdirSync(`./Audio/${speakerId}`, { recursive: true });
...
async function getSpeakerName(speakerId) {
  const uri = process.env.voicevox_api_uri || "http://localhost:50021";
  const speakersRes = await fetch(`${uri}/speakers`);
    ...
    files.forEach((file) => {
      if (file.endsWith(".wav")) {
        if (file === "startWorking.wav") {
          voiceList[
            speakerId
          ].audio.startWorking = `${speakerPath}/startWorking.wav`;
        } else if (file === "startBreaking.wav") {
          voiceList[
            speakerId
          ].audio.startBreaking = `${speakerPath}/startBreaking.wav`;
        } else if (file === "startLongBreaking.wav") {
          voiceList[
            speakerId
          ].audio.startLongBreaking = `${speakerPath}/startLongBreaking.wav`;
        }
      }
    });
  }

  fs.writeFileSync(`./Audio/voiceList.json`, JSON.stringify(voiceList));
}

async function list() {
  // ./Audio/voiceList.jsonから音声ファイルの一覧を取得
  const voiceList = JSON.parse(
    fs.readFileSync("./Audio/voiceList.json", "utf8")
  );
  return voiceList;
}
...
module.exports = { create, sync, list, reset };
