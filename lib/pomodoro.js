...
async function start(client, interaction, options) {
  ...
  const { workTime, breakTime, longBreakTime, voiceNotification } = options;
  ...
  while (!pomodoroState.isCancelled) {
    ...
    await waitOrCancel(workTime, client, interaction);
    ...
    if (pomodoroState.currentCycle % cyclesBeforeLongBreak === 0) {
      ...
      await waitOrCancel(longBreakTime, client, interaction);
    } else {
      await interaction.followUp(`休憩時間 ${breakTime}分 開始！`);
      if (voiceNotification)
        await notifyVoice(client, interaction, "休憩時間開始！");
      await waitOrCancel(breakTime, client, interaction);
    }
    if (pomodoroState.isCancelled) break;
  }
...
// VC監視付きの待機
async function waitOrCancel(minutes, client, interaction) {
