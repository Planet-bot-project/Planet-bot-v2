...
async function start(client, interaction, options) {
  ...
  while (pomodoroState.running) {
    ...
    if (voiceNotification) await notifyVoice(interaction, "startWorking");
    ...
    if (pomodoroState.currentCycle % cyclesBeforeLongBreak === 0) {
      ...
      if (voiceNotification)
        await notifyVoice(interaction, "startLongBreaking");
      ...
      if (voiceNotification) await notifyVoice(interaction, "startBreaking");
  ...
  if (pomodoroState.running) {
    ...
    if (voiceNotification) await notifyVoice(interaction, "stopPomodoro");
...
// ボイス通知
async function notifyVoice(interaction, notifyType) {
  // VC取得
