 ...
 * @param {object} options - オプション（category, tags）
 * @returns {Promise<object>} 作成されたフラッシュカード
 */
async function add(serverId, userId, keyword, definition, options = {}) {
	try {
		const { category = "general", tags = [] } = options;

		// キーワードの重複チェック
		const existingCard = await get(serverId, userId, keyword);
		if (existingCard) {
			throw new Error("このキーワードのフラッシュカードは既に存在します");
		}
...
async function edit(serverId, userId, keyword, updatedFlashcard) {
	try {
		const existingCard = await get(serverId, userId, keyword);
		if (!existingCard) {
			throw new Error("指定されたキーワードのフラッシュカードが見つかりません");
		}
		...
		if (updatedFlashcard.tags !== undefined) {
			updateFields["flashcards.$[user].cards.$[card].tags"] = Array.isArray(
				updatedFlashcard.tags
			)
				? updatedFlashcard.tags
				: [];
		}
 ...
 * @param {string} serverId - サーバーID
 * @param {string} userId - ユーザーID
 * @param {object} filters - フィルター条件（category, tags）
