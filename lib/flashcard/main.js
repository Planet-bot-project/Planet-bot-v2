	...
	try {
		...
		if (updatedFlashcard.category !== undefined) {
			...
			if (!isValidResult.data) {
				return {
					success: false,
					error: `カテゴリー「${updatedFlashcard.category}」は存在しません。先に「/flashcard category create」コマンドでカテゴリーを作成してください。`,
				};
			}
		}
...
async function getRandom(serverId, userId, filters = {}) {
	try {
		const cardsResult = await get(serverId, userId, filters);
		if (
			!cardsResult ||
			!cardsResult.success ||
			!cardsResult.data ||
...
async function getStats(serverId, userId) {
	try {
		const cardsResult = await get(serverId, userId);
		const cards =
