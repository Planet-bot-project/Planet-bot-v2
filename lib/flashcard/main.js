...
async function add(serverId, userId, word, meaning, category) {
	try {
		// キーワードの重複チェック
		const existingCard = await get(serverId, userId, word);
		if (existingCard) {
 ...
 * @param {string} serverId - サーバーID
 * @param {string} userId - ユーザーID
 * @param {string} word - 取得するキーワード
 * @returns {Promise<object|null>} フラッシュカード
 */
async function get(serverId, userId, word) {
	try {
		const server = await profileSchema.findOne(
			{ _id: serverId },
			{ flashcards: 1 }
...
async function edit(serverId, userId, word, updatedFlashcard) {
	try {
		...
		if (result.modifiedCount > 0) {
			return await get(serverId, userId, word);
		}
